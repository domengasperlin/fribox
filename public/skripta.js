window.addEventListener('load', function() {
	...
	var pridobiSeznamDatotek = function(event) {
		...
		xhttp.onreadystatechange = function() {
			if (xhttp.readyState == 4 && xhttp.status == 200) {//smo dobil zadeva dleu
				...
				for (var i=0; i<datoteke.length; i++) {
					...
					datotekeHTML.innerHTML += " <div class='datoteka senca rob'> \
				...
				if (datoteke.length > 0) {//funcionalnost ce nekdo brise
					//document.querySelector("span[akcija=brisi]").addEventListener("click", brisi);//vrne samo en element
